...
namespace ResultAnalyser {
    public class OxyPlottGenerator {
        private const double LineDistance = 60.0;

        private static string[] _types = { "audio", "video", "photo", "text" };
        ...
        public static PlotModel PlotPacketEventDetails(List<NetworkPacket> currPcapFile, List<MsgEvent> currTraceFile) {
            ...
            for (int i = 0; i < _types.Length; i++) {
                var src = currTraceFile.Where(p => p.type.Equals(_types[i])).ToArray();
                if (src.Length == 0) {
                    continue;
                }
                ...
                var scatterSeriesTrace = new ScatterSeries {
                    MarkerType = MarkerType.Plus,
                    MarkerFill = GetMarkerColor(_types[i]),
                    MarkerSize = 6,
                    MarkerStroke = GetMarkerColor(_types[i]),
                    MarkerStrokeThickness = 1.8,
                    Title = "Message " + _types[i]
                };
